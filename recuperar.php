 ...
 if(!filter_var($email,FILTER_VALIDATE_EMAIL)) $err="Email inválido.";
  ...
  try{
   ...
   if($u){
    ...
    $link=(eva_mail_config()['base_url']??'https://dashboard.elvigilantedeagua.com').'/restablecer.php?token='.$token;
...
?>
<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Recuperar contraseña - EVA</title><style>*{margin:0;padding:0;box-sizing:border-box;font-family:'Segoe UI',sans-serif}body{background:#0b1120;min-height:100vh;display:flex;justify-content:center;align-items:center;padding:20px}.card{background:#111c30;padding:40px;border-radius:16px;width:100%;max-width:440px;color:#c8d0dc}h2{color:white;margin-bottom:12px}p{font-size:14px;margin-bottom:16px;color:#8892a4}input{width:100%;padding:12px;background:#0d1525;border:1px solid #1e2d44;border-radius:8px;color:white;margin-bottom:16px}button{width:100%;padding:13px;background:#2563eb;color:white;border:none;border-radius:8px;font-weight:600;cursor:pointer}a{color:#2563eb;text-decoration:none}.msg{background:#065f46;color:white;padding:10px;border-radius:6px;margin-bottom:12px}.err{background:#dc2626;color:white;padding:10px;border-radius:6px;margin-bottom:12px}</style></head><body><div class="card"><h2>Recuperar contraseña</h2><p>Ingresá tu email y te enviaremos un enlace para restablecer tu contraseña.</p><?php if($msg) echo '<div class="msg">'.htmlspecialchars($msg).'</div>'; if($err) echo '<div class="err">'.htmlspecialchars($err).'</div>'; ?><form method="POST"><input type="email" name="email" placeholder="tucorreo@gmail.com" required><button type="submit">Enviar enlace</button></form><p style="margin-top:16px;text-align:center"><a href="index.php">← Volver al login</a></p></div></body></html>
