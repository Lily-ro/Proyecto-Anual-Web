...
if($_SERVER['REQUEST_METHOD']==='POST'){
    try{
        if(isset($_POST['accion_mant']) && isset($_POST['id_mantenimiento'])){
            ...
            if(!isset($map[$accion])) throw new Exception('AcciÃ³n no vÃ¡lida');
            $nuevo = $map[$accion];
            ...
            if(!isset($mapS[$accion])) throw new Exception('AcciÃ³n no vÃ¡lida');
            $nuevo = $mapS[$accion];
...
function fmtFecha($f){
    if(!$f || $f==='0000-00-00 00:00:00') return 'â€”';
    $ts = strtotime($f);
...
<aside class="sidebar">
 ...
 <nav>
  <ul>
   ...
   <li class="anim-slide8" data-toggle="inst"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="7" width="20" height="14" rx="2" ry="2"/><path d="M16 21V5a2 2 0 00-2-2h-4a2 2 0 00-2 2v16"/></svg><span>InstalaciÃ³n</span><span class="arrow">&#9654;</span></li>
   <ul class="sub-menu"><li><a href="instalaciones.php">Lista de instalaciones</a></li></ul>
   <li class="anim-slide9"><a href="historialtecnico.php"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg><span>HistÃ³rico TÃ©cnico</span></a></li>
...
<div class="main">
 <header class="header">
  <div class="header-left" style="display:flex;align-items:center;gap:12px"><button class="menu-btn" aria-label="Abrir menu"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="3" y1="6" x2="21" y2="6"/><line x1="3" y1="12" x2="21" y2="12"/><line x1="3" y1="18" x2="21" y2="18"/></svg></button><div><div class="header-greeting">Mantenimientos de clientes</div><div class="header-subtitle">Mantenimientos asignados con informaciÃ³n del cliente y equipo</div></div></div>
 ...
 <div class="content">
  ...
  <div class="filtros-bar anim-bounce1">
   <input class="form-input" type="text" placeholder="Buscar por cliente, tanque o descripciÃ³n..." id="busquedaMant" oninput="filtrarMantenimientos()">
  ...
  <div class="mant-lista" id="listaMantenimientos">
   <?php foreach($mantenimientos as $idx=>$row):
        ...
        if(empty($cliNombre)) $cliNombre='â€”'; if(empty($cliApellido)) $cliApellido=''; if(empty($cliEmail)) $cliEmail='â€”';
        $direccion = dirCliente($row); if($direccion==='â€”' && !empty($row['cli2_calle'])) $direccion=trim($row['cli2_calle'].' '.$row['cli2_numero'].', '.$row['cli2_localidad']);
        $tanqueTxt = htmlspecialchars($row['tanque_nombre'] ?? 'Sin tanque'); $dispTxt = htmlspecialchars($row['dispositivo_nombre'] ?? ('ID '.$row['id_dispositivo'])); $equipoTxt = $tanqueTxt.' â€” '. $dispTxt . ' (#'.$row['id_dispositivo'].')'; if(!empty($row['id_tanque'])) $equipoTxt .= ' Â· Tanque #'.$row['id_tanque'];
        $tipoTxt = mantTipoTxt($row['tipo']); $estadoNorm = str_replace('_','-',strtolower($row['estado'])); $tipoNorm = strtolower($row['tipo']); $tecTxt = trim(($row['tec_nombre']??'').' '.($row['tec_apellido']??'')); if($tecTxt==='') $tecTxt='â€”'; $fechaProg = fmtFecha($row['fecha_programada']); $fechaReal = fmtFecha($row['fecha_realizada']); $proxFecha = $row['fecha_realizada'] ? $fechaReal : 'â€”'; $desc = $row['descripcion'] ?? ''; $obs = $row['observaciones'] ?? '';
   ?>
   <div class="mant-card anim-bounce<?php echo $idx%4; ?>" data-estado="<?php echo $estadoNorm; ?>" data-tipo="<?php echo $tipoNorm; ?>">
    <div class="mant-card-header" onclick="toggleMantCard(this)"><div class="mant-card-info"><div class="mant-card-titulo"><?php echo htmlspecialchars(trim($cliNombre.' '.$cliApellido)); ?> â€” <?php echo htmlspecialchars($tipoTxt); ?></div><div class="mant-card-meta"><span><?php echo $equipoTxt; ?></span><span><?php echo htmlspecialchars($tipoTxt); ?></span><span><?php echo $fechaProg; ?></span></div></div><?php echo mantBadgeEstadoTec($row['estado']); ?></div>
    <div class="mant-card-body">
     <div class="mant-cliente-grid"><div class="mant-cliente-item"><span class="mant-cliente-label">Cliente</span><span class="mant-cliente-val"><?php echo htmlspecialchars(trim($cliNombre.' '.$cliApellido)); ?></span></div><div class="mant-cliente-item"><span class="mant-cliente-label">Email / Usuario</span><span class="mant-cliente-val"><?php echo htmlspecialchars($cliEmail); ?></span></div><div class="mant-cliente-item"><span class="mant-cliente-label">TelÃ©fono</span><span class="mant-cliente-val"><?php echo htmlspecialchars($row['cli_telefono'] ?? 'â€”'); ?></span></div><div class="mant-cliente-item"><span class="mant-cliente-label">DirecciÃ³n</span><span class="mant-cliente-val"><?php echo htmlspecialchars($direccion); ?></span></div><div class="mant-cliente-item"><span class="mant-cliente-label">Tanque / Equipo</span><span class="mant-cliente-val"><?php echo $equipoTxt; ?></span></div><div class="mant-cliente-item"><span class="mant-cliente-label">Edificio</span><span class="mant-cliente-val"><?php echo htmlspecialchars($row['edificio_nombre'] ?? 'â€”'); ?> <?php echo $row['edificio_direccion'] ? 'Â· '.htmlspecialchars($row['edificio_direccion']) : ''; ?></span></div><div class="mant-cliente-item"><span class="mant-cliente-label">Fecha programada</span><span class="mant-cliente-val"><?php echo $fechaProg; ?></span></div><div class="mant-cliente-item"><span class="mant-cliente-label">PrÃ³xima fecha / Realizada</span><span class="mant-cliente-val"><?php echo $proxFecha; ?></span></div><div class="mant-cliente-item"><span class="mant-cliente-label">Tipo</span><span class="mant-cliente-val"><?php echo htmlspecialchars($tipoTxt); ?></span></div><div class="mant-cliente-item"><span class="mant-cliente-label">TÃ©cnico asignado</span><span class="mant-cliente-val"><?php echo htmlspecialchars($tecTxt); ?> (<?php echo htmlspecialchars($row['tec_email'] ?? ''); ?>)</span></div><div class="mant-cliente-item"><span class="mant-cliente-label">Estado</span><span class="mant-cliente-val"><?php echo mantBadgeEstadoTec($row['estado']); ?></span></div><div class="mant-cliente-item"><span class="mant-cliente-label">Costo</span><span class="mant-cliente-val">$<?php echo number_format((float)$row['costo'],2,',','.'); ?></span></div></div>
     <div class="mant-divider"></div>
     <div class="form-group"><label class="form-label">DescripciÃ³n</label><p style="font-size:13px;color:var(--tx);line-height:1.5"><?php echo nl2br(htmlspecialchars($desc ?: 'â€”')); ?></p></div>
     <div class="form-group"><label class="form-label">Observaciones</label><p style="font-size:13px;color:var(--tx);line-height:1.5"><?php echo nl2br(htmlspecialchars($obs ?: 'â€”')); ?></p></div>
     <?php if(!empty($row['sm_id'])): ?><div class="mant-prox">Solicitud vinculada #<?php echo (int)$row['sm_id']; ?> Â· <?php echo htmlspecialchars($row['sm_desc'] ?? ''); ?></div><?php endif; ?>
     <div class="mant-divider"></div>
     <form method="POST" style="display:flex;flex-direction:column;gap:10px">
      ...
      <textarea name="observaciones" rows="2" class="form-input" placeholder="AÃ±adir observaciÃ³n al resolver (opcional)"></textarea>
  ...
  <div class="mant-lista" id="listaSolicitudes">
   <?php foreach($solicitudes as $idx=>$s):
        $sn = trim(($s['cli_nombre']??'').' '.($s['cli_apellido']??'')); if($sn==='') $sn='Usuario #'.($s['id_usuario']??'â€”');
        $sDir = trim(($s['cli_calle']??'').' '.($s['cli_numero']??'')); if($sDir==='') $sDir=$s['edificio_direccion'] ?? $s['tanque_ubicacion'] ?? 'â€”'; if(!empty($s['cli_localidad'])) $sDir .= ', '.$s['cli_localidad'];
   ...
   <div class="mant-card" data-estado="<?php echo $sEstadoNorm; ?>" data-tipo="solicitud">
    <div class="mant-card-header" onclick="toggleMantCard(this)"><div class="mant-card-info"><div class="mant-card-titulo">Solicitud #<?php echo (int)$s['id_solicitud']; ?> â€” <?php echo htmlspecialchars($sn); ?></div><div class="mant-card-meta"><span><?php echo htmlspecialchars($s['tanque_nombre'] ?? 'Tanque #'.$s['id_tanque']); ?></span><span>Solicitud</span><span><?php echo fmtFecha($s['fecha_solicitud']); ?></span></div></div><span class="badge <?php echo $sBadge; ?>"><?php echo htmlspecialchars($sEstado); ?></span></div>
    <div class="mant-card-body">
     <div class="mant-cliente-grid"><div class="mant-cliente-item"><span class="mant-cliente-label">Cliente</span><span class="mant-cliente-val"><?php echo htmlspecialchars($sn); ?></span></div><div class="mant-cliente-item"><span class="mant-cliente-label">Email</span><span class="mant-cliente-val"><?php echo htmlspecialchars($s['cli_email'] ?? 'â€”'); ?></span></div><div class="mant-cliente-item"><span class="mant-cliente-label">TelÃ©fono</span><span class="mant-cliente-val"><?php echo htmlspecialchars($s['cli_telefono'] ?? 'â€”'); ?></span></div><div class="mant-cliente-item"><span class="mant-cliente-label">DirecciÃ³n</span><span class="mant-cliente-val"><?php echo htmlspecialchars($sDir); ?></span></div><div class="mant-cliente-item"><span class="mant-cliente-label">Tanque / Equipo</span><span class="mant-cliente-val"><?php echo htmlspecialchars(($s['tanque_nombre'] ?? 'Tanque #'.$s['id_tanque']).' Â· '.($s['edificio_nombre'] ?? '')); ?></span></div><div class="mant-cliente-item"><span class="mant-cliente-label">TÃ©cnico asignado</span><span class="mant-cliente-val"><?php echo htmlspecialchars($sTec); ?></span></div><div class="mant-cliente-item"><span class="mant-cliente-label">Fecha solicitud</span><span class="mant-cliente-val"><?php echo fmtFecha($s['fecha_solicitud']); ?></span></div><div class="mant-cliente-item"><span class="mant-cliente-label">PrÃ³xima fecha / Finalizada</span><span class="mant-cliente-val"><?php echo fmtFecha($s['fecha_finalizada']); ?></span></div><div class="mant-cliente-item"><span class="mant-cliente-label">Estado</span><span class="mant-cliente-val"><span class="badge <?php echo $sBadge; ?>"><?php echo htmlspecialchars($sEstado); ?></span></span></div><div class="mant-cliente-item"><span class="mant-cliente-label">ID Tanque</span><span class="mant-cliente-val">#<?php echo (int)$s['id_tanque']; ?></span></div></div>
     <div class="mant-divider"></div>
     <div class="form-group"><label class="form-label">DescripciÃ³n / Observaciones</label><p style="font-size:13px;color:var(--tx);line-height:1.5"><?php echo nl2br(htmlspecialchars($s['descripcion'] ?? 'â€”')); ?></p></div>
     <form method="POST" style="display:flex;flex-direction:column;gap:10px">
      <input type="hidden" name="id_solicitud" value="<?php echo (int)$s['id_solicitud']; ?>">
      <label class="form-label">AcciÃ³n del tÃ©cnico â€” Resolver solicitud</label>
      <textarea name="observaciones_sol" rows="2" class="form-input" placeholder="ObservaciÃ³n tÃ©cnica (opcional)"></textarea>
      ...
      <input type="text" name="descripcion_final" class="form-input" placeholder="DescripciÃ³n para mantenimiento (si marca la casilla)">
      <div style="display:flex;gap:8px;flex-wrap:wrap">
